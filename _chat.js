...
(function () {
  ...
  var CHANNELS = [
    ...
    //
    // **ปิดไว้ก่อน — ยังไม่รู้ที่อยู่จริงของเพจ**: ทั้ง m.me/doospend และ
    // facebook.com/doospend ใช้ไม่ได้ ("doospend" น่าจะเป็น *ชื่อ* เพจ ไม่ใช่ *ชื่อผู้ใช้*
    // ที่อยู่หลัง facebook.com/) · ตรวจจากข้างนอกไม่ได้เพราะ Facebook บังคับล็อกอิน
    // ทุกทาง — ต้องเอาลิงก์จากปุ่มแชร์ของเพจมาใส่ตรงนี้
    //
    // เดิม m.me/doospend ไม่ redirect (ตอบ 200 เฉยๆ) = แปลงชื่อผู้ใช้เป็น
    // ห้องแชทไม่ได้ กดแล้วเปิดแอป Messenger ค้างที่หน้าแรก ไม่เข้าแชท (เทียบกับเพจที่ใช้ได้
    // เช่น m.me/facebook จะ 302 ไป m.facebook.com/msg/<id>) มักเกิดจากเพจปิดรับข้อความ
    // หรือ vanity ยังไม่ผูกกับกล่องข้อความ
    //
    // ระหว่างนี้ชี้ไปหน้าเพจ ซึ่งมีปุ่ม "ส่งข้อความ" อยู่แล้ว — เสียหนึ่งแตะ แต่ไปถึงจริง
    // ดีกว่าปุ่มที่กดแล้วไปตัน. พอได้ **รหัสเพจ (ตัวเลข)** ให้เปลี่ยนเป็น
    // 'https://m.me/<PAGE_ID>' แล้วเปลี่ยน label กลับเป็น 'คุยทาง Messenger'
    { id: 'messenger', label: 'ทักเพจ Facebook', url: '', color: '#0866FF',
